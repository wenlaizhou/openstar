...
local function or_remath(_or_list, _basemsg)
    ...
    for _, v in ipairs(_or_list) do
        if action_remath(v[1], v[2], _basemsg) then
            -- 真
            return true
        end
    end
    ...
    for i, v in ipairs(_app_list) do
        if v[3] == "or" then
            ...
            if #tmp_or == 0 then
                ...
                -- 一组 or 计算
                table.insert(tmp_or, v)
                if or_remath(tmp_or, _basemsg) then
                    -- 真
                    -- continue
                else
                    return false
...
local function get_postByName(_name)
    ...
    if type(posts_name) == "table" then
        posts_name = posts_name[1]
    end
